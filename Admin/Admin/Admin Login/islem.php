...
if(isset($_POST['kayit'])){
    ...
    elseif($password != $password_again){
       ...
       $sorgu=$db->prepare('INSERT INTO adminn SET admin_name=?,admin_password=?') ;
       $ekle=$sorgu->execute([
        $username,$password
       ]);
       if($ekle){
...
if(isset($_POST['giris'])){
    ...
    elseif(!$password){
        ...
        $kullanici_sor=$db->prepare('SELECT * FROM adminn WHERE admin_name=? || admin_password=?');
